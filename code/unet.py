#Font: https://github.com/EdwardTyantov/ultrasound-nerve-segmentation/blob/master/u_model.py
...
def get_unet_inception_2head(optimizer, img_rows, img_cols, weights_path):
    splitted = True
    act = 'elu'
    ...
    pre = Flatten()(pre)
    aux_out = Dense(8, activation='sigmoid', name='aux_output')(pre) 
    '''
    #
...
def train_mask_detector(img_rows, img_cols, weights_path):
    from keras.optimizers import Adam, RMSprop, SGD
    
    optimizer = RMSprop(lr=0.045, rho=0.9, epsilon=1.0)
    model = get_unet(Adam(lr=1e-5), img_rows, img_cols, weights_path)
